...
export default function StayCalculator() {
  ...
  useEffect(() => {
    if (rentals.length > 0 && selectedId === null) {
      setSelectedId(rentals[0].id);
    }
  }, [rentals]);
              ...
              <div>
                ...
                <div className="space-y-3">
                  {rentals.map((r) => (
                    ...
                    >
                      <div>
                        <p className={`text-sm font-body ${selectedId === r.id ? "text-[#0C4A6E]" : "text-[#0C4A6E]/60"}`}>
                          {r.name}
                        </p>
                        {r.type && (
                      ...
                      <span className={`font-heading text-lg font-light ${selectedId === r.id ? "text-[#0891B2]" : "text-[#0C4A6E]/30"}`}>
                        {r.price}€
                        <span className="text-[10px] font-body font-normal text-[#0C4A6E]/30">/nuit</span>
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-[#F7F5F2]/55 text-sm font-body">
                    {selected?.price}€ × {nights} nuit{nights > 1 ? "s" : ""}
                  </span>
